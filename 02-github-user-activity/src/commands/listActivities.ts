import { GitHubUserActivity } from "../types/user.js";
import { readUsers } from "../utils/fileSystem.js";

export const listUserActivities = (filter?: string): void => {
  ...
  if (filter) {
    ...
    if (!validFilters.includes(filter.toLowerCase())) {
      console.error(
        `Error: Invalid activity type filter. Valid options: ${validFilters.join(", ")}`,
      );
      process.exit(1);
    }
  }
